//! TSON to GGRS DataFrame conversion
//!
//! Converts TSON binary format (Tercen's native format) to GGRS DataFrame.
...
/// Convert TSON bytes to GGRS DataFrame
///
/// This function decodes TSON binary format and converts it to a GGRS DataFrame.
/// The TSON data is expected to represent a table structure.
pub fn tson_to_dataframe(tson_bytes: &[u8]) -> Result<DataFrame> {
...
/// Convert a TSON value to GGRS DataFrame
///
...
/// }
/// ```
fn tson_value_to_dataframe(tson: &TsonValue) -> Result<DataFrame> {
    ...
    Ok(DataFrame::from_polars(polars_df))
}
...
// NOTE: Dead functions removed - we now use pure columnar Polars operations
// No more row-by-row Record building!
